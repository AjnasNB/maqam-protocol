...
export async function evaluateProposal(input) {
  ...
  const gateway = new ToolGateway({
    policyEngine: new PolicyEngine({ allowedTools: ["payment.evaluate"] }),
  });
  gateway.registerTool(
    "payment.evaluate",
    async (value) => assessProposal(value),
    { effects: ["read"] },
  );
  return {
    assessment: await gateway.call("payment.evaluate", input, {
      runId: crypto.randomUUID(),
    }),
    governance: {
      library: "maqam",
      version: "0.3.3",
      tool: "payment.evaluate",
      boundary: "proposal policy; execution requires onchain signature",
    },
  };
}
...
export function createPaymentGateway(handler) {
  ...
  const gateway = new ToolGateway({
    policyEngine: new PolicyEngine({
      allowedTools: ["payment.execute"],
      approvalRequiredEffects: ["send"],
    }),
